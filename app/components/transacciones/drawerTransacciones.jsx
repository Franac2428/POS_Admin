...
export default function DrawerTransacciones({ transaction, article, paytype, price, status }) {
    return (
        <>
            {/* drawer component */}
            <li>
                <button data-drawer-target={`${transaction}-drawer-update-product`} data-drawer-show={`${transaction}-drawer-update-product`} aria-controls={`${transaction}-drawer-update-product`} className="block py-2 px-4 w-full text-left hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white" >
                    Edit
                </button>
            </li>
            <form action="#" id={`${transaction}-drawer-update-product`} className="absolute top-0 left-0 z-40 w-full h-screen max-w-3xl p-4 overflow-y-auto transition-transform -translate-x-full bg-white dark:bg-gray-800" tabIndex={-1} aria-labelledby="drawer-update-product-label" aria-hidden="true" >
                <h5
                    id="drawer-label"
                ...
                <button
                    type="button"
                    data-drawer-dismiss={`${transaction}-drawer-update-product`}
                    aria-controls={`${transaction}-drawer-update-product`}
